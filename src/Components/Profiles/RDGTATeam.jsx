...
export default function RDGTATeam() {
  ...
  const [form, setForm] = useState({
    ...
    candidateName: "",
    emailId: "",
    countryId: "1", // UPDATED: default to India (+91); dynamic fallback below will correct if ID differs
    phoneNumber: "",
    experience: "",
    skillCluster: null,
    primarySkills: [],
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ UPDATED: Ensure +91 is selected by default after dropdowns load
        // If current form.countryId not found in list, or empty -> set to +91 by callingCode
  ...
  const roleNm = getCurrentRoleName(); // used for showEmpIdField already
          ...
          <div className={`${grid2} ${sectionGap}`}>
            <div>
              <div className="flex items-center gap-2 mt-1">
                ...
                >
                  {/* ✅ UPDATED: removed static placeholder "+91"; real options only */}
                  {countryCodes.map((c) => (
                    <option key={c.id} value={c.id}>
                      {c.callingCode}
                    </option>
