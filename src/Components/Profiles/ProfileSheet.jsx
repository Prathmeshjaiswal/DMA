...
// /* ------------ Demand normalizer for attach list ------------ */
// const nameOf = (obj) => (obj && typeof obj === "object" ? (obj.name ?? "") : String(obj ?? ""));
// const joinNames = (arr) => (Array.isArray(arr) ? arr.map(nameOf).filter(Boolean).join(", ") : nameOf(arr));
// function normalizeDemandForAttach(d) {
//   const id = d.id ?? d.demandId ?? d.displayDemandId ?? Math.random();
//   return {
//     id, // this is the demand PK (from your demandsheet list)
//     demandId: d.displayDemandId ?? d.demandId ?? String(id), // for display
//     hbu: nameOf(d.hbu),
...
// };
// // pretty day+date+time for cards
// const formatDayDateTime = (v) => {
//   if (!v) return "-";
//   try {
//     const d = new Date(v);
...
//   /* -------- attach (inline) -------- */
//   const [matchingDemands, setMatchingDemands] = useState([]);      // HBU matches
//   const [selectedDemandIds, setSelectedDemandIds] = useState([]);  // ids checked in list
//   const [attachedDemands, setAttachedDemands] = useState([]);      // backend-attached (with dates)
//   const [attachMode, setAttachMode] = useState(false);             // list vs cards

//   // meta index to enrich attached cards with cluster/primary
...
//   // ---- Build server-side filter for profile list ----
//   const buildServerFilter = useCallback(() => {
//     const filter = {};

//     if (query.candidateName) filter.candidateName = String(query.candidateName).trim();
//     if (query.emailId)       filter.emailId       = String(query.emailId).trim();
//     if (query.empId)         filter.empId         = String(query.empId).trim();

//     if (query.phoneNumber && /^\d+$/.test(String(query.phoneNumber))) {
//       filter.phoneNumber = Number(query.phoneNumber);
//     }

//     if (query.experienceYears && !Number.isNaN(Number(query.experienceYears))) {
//       const exp = Number(query.experienceYears);
//       filter.minExperience = exp;
//       filter.maxExperience = exp;
//     }

//     if (query.skillCluster) filter.skillClusterName = String(query.skillCluster).trim();
//     if (query.location)     filter.locationName     = String(query.location).trim();
//     if (query.hbu)          filter.hbuName          = String(query.hbu).trim();

//     if (query.primarySkills) {
...
//     if (query.summary) filter.summary = String(query.summary).trim();
...
//       // Map the server DTO to card model
//       const mapped = list.map((x) => {
//         const key = String(x.demandId ?? x.id ?? "");
//         const meta = key ? metaIndex[key] : undefined;
//         // Server sample shows: id = tracker id, demandId = demand PK
//         return {
//           trackerId: x.id,
//           id: x.demandId ?? x.id,
...
//       setAttachMode(mapped.length === 0); // show list only when there are no attachments
...
//   // ===== Matching (HBU only) + meta index =====
//   const loadMatchingDemands = useCallback(async (row) => {
//     if (!row) { setMatchingDemands([]); return; }
//     const hbuNeed = String(row.hbu || "").trim().toLowerCase();

//     try {
//       const resp = await getDemandsheet(0, 200);
//       const list =
...
//       const normalized = list.map((d) => normalizeDemandForAttach(d));

//       // Build/merge meta index
//       const metaEntries = {};
...
//         (d) => String(d.hbu || "").trim().toLowerCase() === hbuNeed
//       );

//       // Also remove any that are already attached (by demand id)
//       setMatchingDemands((prev) => {
//         const already = new Set(attachedDemands.map((a) => String(a.id)));
//         return filtered.filter((f) => !already.has(String(f.id)));
//       });
//     } catch (err) {
...
//         return;
//       }
//       loadAttachedFromBackend(numericProfileId).then(() => {
//         // after we know what's attached, fetch matches and exclude attached
//         loadMatchingDemands(detailRow);
//       });
//     }
...
//   /* ---- ATTACH via backend, OPTIMISTIC CARDS, then REFRESH ---- */
...
//       // --- Optimistic cards (instant switch to cards)
...
//       // Merge optimistic into attached
...
//       // Remove attached demands from the list immediately
//       setMatchingDemands((prev) => prev.filter((d) => !selSet.has(String(d.id))));
//       setSelectedDemandIds([]);
//       setAttachMode(false);

//       // Refresh from backend to replace optimistic with real tracker rows
//       await loadAttachedFromBackend(profileId);

//       // Recompute the list after refresh (still excluding attached)
...
//                                   {d.createdAt ? formatDayDateTime(d.createdAt) : formatDayDateTime(d.attachedDate)}
//                                 </div>
//                               </div>
...
//                               )}
//                               {!!d.hbu && (
//                                 <div className="text-[12px] text-gray-600">HBU: {d.hbu}</div>
//                               )}
//                               {!!d.skillCluster && (
//                                 <div className="text-[12px] text-gray-600">Skill Cluster: {d.skillCluster}</div>
//                               )}
//                               {!!d.primarySkills && (
//                                 <div className="text-[12px] text-gray-600">Primary: {d.primarySkills}</div>
//                               )}
//                             </div>
//                           ))}
//                         </div>
...
//                         <div><strong>Created At:</strong> {formatDayDateTime(detailRow.createdAt)}</div>
//                         <div><strong>Updated By (User ID):</strong> {detailRow.updatedByUserId || "-"}</div>
//                         <div><strong>Updated At:</strong> {formatDayDateTime(detailRow.updatedAt)}</div>
//                         <div>
