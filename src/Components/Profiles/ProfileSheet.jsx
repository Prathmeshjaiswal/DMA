...
// ✅ Date-only (no time) — for attached demand cards & history
...
function toneStyles(tone) {
  switch (tone) {
    case 'green': return { borderColor: '#16a34a', backgroundColor: '#f0fdf4' };
    case 'red': return { borderColor: '#dc2626', backgroundColor: '#fef2f2' };
    default: return { borderColor: '#f59e0b', backgroundColor: '#fffbeb' };
  }
}
...
// Reusable label/value row (bold labels, left aligned everywhere)
...
function OnboardingDetailSection({ onboarding, demandId, profileId }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm" style={{ textAlign: "left" }}>
      {/* ↓↓↓ reduced middle whitespace: gap-x-4 instead of 8 */}
...
export default function ProfileSheet() {
  ...
  useEffect(() => { fetchServer(0, size); /* eslint-disable-line */ }, []);
    ...
    try {
      ...
      const mapped = list.map((x) => {
        ...
        const primarySkillsStr = (() => {
          if (meta?.primarySkills) return meta.primarySkills;
  ...
  // Load attached + matching in parallel (minimize flicker)
  useEffect(() => {
    if (!detailOpen || activeViewTab !== "demand" || !detailRow) return;
            ...
            items={[
              {
                ...
                children: (
                  // ✅ SAME UI as Demand Details, with reduced middle gap and bold labels
                  <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm text-sm" style={{ textAlign: "left" }}>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4" style={{ justifyItems: "start" }}>
                      {/* LEFT */}
                      <div className="space-y-1.5 leading-tight">
                        <RowL label="Candidate" value={detailRow.candidateName || "-"} />
                        <RowL label="Email" value={detailRow.emailId || "-"} />
                        <RowL label="Phone" value={detailRow.phoneNumber || "-"} />
                        <RowL label="Experience" value={detailRow.experienceYears || "-"} />
                        <RowL label="Location" value={detailRow.location || "-"} />
                        <RowL label="HBU" value={detailRow.hbu || "-"} />
                      </div>
                      {/* RIGHT */}
                      <div className="space-y-1.5 leading-tight">
                        <RowL label="Skill Cluster" value={detailRow.skillCluster || "-"} />
                        <RowL label="Primary Skills" value={detailRow.primarySkills || "-"} />
                        <RowL label="Secondary Skills" value={detailRow.secondarySkills || "-"} />
                        <RowL label="Employee ID" value={detailRow.empId || "-"} />
                        <RowL label="Summary" value={detailRow.summary || "-"} />
                      </div>
                    </div>
                  </div>
                ),
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {attachedDemands.map((d) => {
                            ...
                            return (
                              ...
                              >
                                {/* Top row: Demand # + date only (no time) */}
...

              {
                ...
                <div className="p-1 text-left">
                  <HistoryPanel mode="profile" entityId={detailRow.id} />
                </div>
                ),
              },
            ]}
