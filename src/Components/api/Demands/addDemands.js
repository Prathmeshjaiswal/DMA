...
// ✅ UPDATED API helper: matches controller expecting @RequestPart("payload")

import api from '../client';

// === Step-1 (Draft) ===
const STEP1_PART = 'req'; // keep as your controller expects: 'req' or 'request'

export const submitStep1 = async (requestPayload = {}) => {
  try {
    const fd = new FormData();
    fd.append(STEP1_PART, new Blob([JSON.stringify(requestPayload)], { type: 'application/json' }));
    const res = await api.post('/draft/create', fd, { headers: {} });
    return res.data;
  } catch (err) {
    const data = err?.response?.data;
    const msg = data?.message || data?.detail || data?.title || err?.message || 'Step-1 failed.';
...
export const updateStep1Draft = async (draftId, requestPayload = {}) => {
  try {
    const fd = new FormData();
    fd.append(STEP1_PART, new Blob([JSON.stringify(requestPayload)], { type: 'application/json' }));
    const res = await api.put(`/edit/${draftId}`, fd, { headers: {} });
    return res.data;
  } catch (err) {
    const data = err?.response?.data;
    const msg = data?.message || data?.detail || data?.title || err?.message || 'Draft update failed.';
...
// === Step-2 (Final submit) ===
// 🔴 Controller expects: @RequestPart("payload")
const STEP2_JSON_PART = 'payload';
...
export const submitStep2 = async (payload) => {
  ...
  try {
    if (payload instanceof FormData) {
      // If caller already created FormData, keep it as-is.
      formData = payload;
    } else {
      const {
        addNewDemandDTO, // MUST include karatFlag=1/0
        file = null,
        files = [],
        filesMeta = null,
      } = payload || {};
      ...
      // ✅ IMPORTANT: send JSON under 'payload' to match the controller
      formData.append(
        STEP2_JSON_PART,
        new Blob([JSON.stringify(addNewDemandDTO)], { type: 'application/json' })
      );

      // Optional: include meta for files mapping (kept same)
      let meta = filesMeta;
      if (!meta && Array.isArray(addNewDemandDTO?.demandRRDTOList)) {
        meta = addNewDemandDTO.demandRRDTOList.map((item, index) => ({
          index,
