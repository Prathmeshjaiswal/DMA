...
export default function AddDemands2() {
  ...
  // Save draft (RR + mode + text)
  const saveDraft = () => {
    try {
      const data = {
        byId: demandIds.reduce((acc, id, idx) => {
          acc[id] = {
            rr: rrNumbers[idx] ?? '',
            mode: rowMode[idx] ?? '',
            text: rowText[idx] ?? '',
...
//   }, [form1Data]);
//
//   // RR numbers (no +/- spinner: plain text numeric)
//   const [rrNumbers, setRrNumbers] = useState([]); // idx -> string digits
//
//   const onChangeRR = (index, value) => {
//     const cleaned = value.replace(/\D/g, '');
...
//
//   // Per-row state
//   const [rowFiles, setRowFiles] = useState({});        // idx -> File to submit (OCR/text handled)
//   const [rowRawFiles, setRowRawFiles] = useState({});  // idx -> original file selected
//   const [rowErrors, setRowErrors] = useState({});      // idx -> error string
//   const [rowMode, setRowMode] = useState({});          // idx -> 'file' | 'text'
//   const [rowText, setRowText] = useState({});          // idx -> string (max 5000)
//
...
//   };
//
//   // === OCR: if image, extract text and convert to .txt File ===
//   const ocrImageToTextFile = async (file) => {
//     try {
//       const T = await import('tesseract.js'); // npm i tesseract.js
//       const { data: { text } } = await T.recognize(file, 'eng');
//       const blob = new Blob([text || ''], { type: 'text/plain' });
//       const txtName = file.name.replace(/\.[^/.]+$/, '') + '.txt';
//       try {
//         return new File([blob], txtName, { type: 'text/plain' });
//       } catch {
//         blob.lastModifiedDate = new Date();
//         blob.name = txtName;
//         return blob;
//       }
//     } catch (err) {
//       console.warn('OCR failed or tesseract.js not available; using original file.', err);
...
//     if (isImage) {
//       finalFile = await ocrImageToTextFile(f);
//     }
//     setRowErrors((prev) => ({ ...prev, [idx]: '' }));
//     setRowRawFiles((prev) => ({ ...prev, [idx]: f }));
//     setRowFiles((prev) => ({ ...prev, [idx]: finalFile }));
//   };
//
//   const onFileChange = async (idx, e) => {
//     const f = e.target.files?.[0] || null;
//     if (!f) return;
//     setRowMode((prev) => ({ ...prev, [idx]: 'file' })); // set mode to file
//     await handleSelectedFile(idx, f);
//   };
//
//   // Drag & drop area handlers
...
//       if (!draft?.byId) return;
//
//       // Rehydrate by current demand order
//       const nextRR = [];
//       const nextMode = {};
//       const nextText = {};
//       demandIds.forEach((id, idx) => {
...
//     setSubmitting(true);
//     try {
//       // Build DTO: pair each demand with RR
//       const demandRRDTOList = demandIds.map((id, idx) => ({
//         demandId: id,
//         rrNumber: Number(rrNumbers[idx] || 0),
//       }));
...
//       };
//
//       // Build multipart with multiple files (one per demand)
//       const formData = new FormData();
//       formData.append(
//         'addNewDemandDTO',
//         new Blob([JSON.stringify(addNewDemandDTO)], { type: 'application/json' })
...
//
//       // Attach per-row file:
//       // - if mode=file: the (possibly OCR’d) file
//       // - if mode=text: create a .txt file from the entered text
//       const filesMeta = [];
//       for (let idx = 0; idx < demandIds.length; idx++) {
//         const id = demandIds[idx];
//         let f = null;
//
//         if (rowMode[idx] === 'file') {
//           f = rowFiles[idx]; // already OCR converted if image
//         } else {
//           // build a text file from rowText
//           const content = (rowText[idx] || '').trim();
//           const blob = new Blob([content], { type: 'text/plain' });
//           const fname = `JD_${id}.txt`;
//           try {
...
//       formData.append('filesMeta', JSON.stringify(filesMeta));
//
//       // Post: submitStep2 accepts FormData
//       const res = await submitStep2(formData);
//       const serverResponse = res?.data ?? res;
//       message.success('Demand has been created successfully!');
//       // Clear draft
...
//               {/* Header row */}
//               <div className="px-6 pt-6">
//                 <div className="grid grid-cols-[180px_60px_180px_1fr] items-center gap-4">
//                   <div className="text-gray-700 font-medium">Generated ID</div>
//                   <div />
//                   <div className="text-gray-700 font-medium">Enter RR No.</div>
//                   <div className="text-gray-700 font-medium">JD Input</div>
...
//                         <div
//                           key={id}
//                           className="grid grid-cols-[180px_60px_180px_1fr] items-center gap-4 rounded-lg bg-white p-4 border border-gray-200 shadow-sm"
//                           onDragOver={onDragOver}
//                           onDrop={(e) => onDrop(idx, e)}
//                         >
//                           {/* Generated Demand ID (readonly) */}
//                           <input
//                             type="text"
//                             value={id}
//                             readOnly
...
//                       demandIds.some((_, idx) => !!rowErrors[idx]) ||
//                       demandIds.some((_, idx) => {
//                         // if file mode: must have file; if text mode: must have non-empty text
//                         const mode = rowMode[idx];
//                         if (!mode) return true;
//                         if (mode === 'file') return !rowFiles[idx];
//                         if (mode === 'text') return !(rowText[idx] || '').trim();
