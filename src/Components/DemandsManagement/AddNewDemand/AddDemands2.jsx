...
export default function AddDemands2() {
  ...
  const passedKaratFlag = state?.karatFlag; // ✅ UPDATED: pick karatFlag if Step-1 passed it
    ...
    (async () => {
      try {
        ...
        setMeta({
          ...
          // ✅ UPDATED: carry karatFlag from draft if present
          karatFlag: (data?.karatFlag != null)
            ? Number(data.karatFlag)
            : undefined,
        });
      } catch (e) {
  ...
  const isValidRrNumber = (rr) => {
    return /^\d{6,8}$/.test(String(rr || ""));
  };
  ...
  const onSaveDraft = async () => {
    try {
      ...
      const request = {
        ...
        rrDrafts,
        // (draft save path doesn't need karatFlag)
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ UPDATED: Normalize karatFlag (prefer nav -> meta.karatFlag -> meta.karat "yes/no")
      const normalizedKarat =
        (passedKaratFlag != null && Number.isFinite(Number(passedKaratFlag)))
          ? Number(passedKaratFlag)
          : (meta?.karatFlag != null && Number.isFinite(Number(meta.karatFlag)))
            ? Number(meta.karatFlag)
            : (String(meta?.karat || '').toLowerCase() === 'yes' ? 1 : 0);
