...
const DEMAND_TYPES = ["Current", "Future"];
  ...
  const labelPill = "rounded-md bg-orange-300 text-white px-4 py-1 text-sm font-medium border border-[#52624E]";
...
{/*       <div className="mt-4"><hr className="border-gray-300" /></div> */}
      ...
      <div className="grid grid-cols-6 gap-5 mx-9 px-9">
        ...
        <div className="grid grid-rows-3 gap-1 items-center">
          ...
          <select className={inputBox} value={form.deliveryManager} onChange={(e) => setForm("deliveryManager", e.target.value)}>
            ...
            {DELIVERY_MANAGERS.map((o) => <option key={o} value={o}>{o}>{o}</option>)}
          </select>
        </div>
        <div className="grid grid-rows-3 gap-1 items-center">
          ...
          <select className={inputBox} value={form.demandType} onChange={(e) => setForm("demandType", e.target.value)}>
            ...
            {DEMAND_TYPES.map((o) => <option key={o} value={o}>{o}</option>)}
                      ...
                      <div className="flex items-center gap-4 m-2 px-6">
                        <button onClick={() => navigate("/AddDemands2")} className=" bg-green-900 rounded-md text-white py-2 px-10 font-medium tracking-wide hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-400">
                          Next
                        </button>
                      </div>
      </div>
