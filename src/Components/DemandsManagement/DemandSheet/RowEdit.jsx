...
//         demandLocation: toOptions([
//   ...(dd.onshoreLocationList || []),
//   ...(dd.offshoreLocationList || []),
// ]),

onshoreLocation: toOptions(dd.onshoreLocationList),
offshoreLocation: toOptions(dd.offshoreLocationList),
  ...
  detectedLocationType === "offshore"
    ? opt.offshoreLocation
    : opt.onshoreLocation,
  row.demandLocation
),
      ...
      lobId: ONE(opt.lob, row.lob),
      pmoId: ONE(opt.pmo, row.pmo),
  ...
  const detectedLocationType = useMemo(() => {
  const selectedNames = String(row?.demandLocation || "")
    .split(",")
    .map(s => s.trim().toLowerCase())
    .filter(Boolean);

  if (!selectedNames.length) return "onshore"; // default

  const offshoreSet = new Set(
    (opt.offshoreLocation || []).map(o => o.label.toLowerCase())
  );

  // If ANY selected location is offshore → offshore
  return selectedNames.some(name => offshoreSet.has(name))
    ? "offshore"
    : "onshore";
}, [row?.demandLocation, opt.offshoreLocation]);
  ...
  const renderSelectCell = (colKey, placeholder = "Select…") => {
    ...
    switch (colKey) {
      case "lob":
        options = opt.lob;
        value = sel.lobId;
        onChange = v => setSel(s => ({ ...s, lobId: v ?? null }));
        break;

      case "band":
        options = opt.band;
        value = sel.bandId;
        onChange = v => setSel(s => ({ ...s, bandId: v ?? null }));
        break;
      ...
      <td className="border-b border-gray-200 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={isSaving}
            className={`inline-flex items-center rounded-md px-3 py-1.5 text-sm font-medium text-white shadow-sm ${isSaving ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"
              }`}
          >
          ...
          <button
            ...
            onClick={cancelEdit}
            disabled={isSaving}
            className={`inline-flex items-center rounded-md px-3 py-1.5 text-sm font-medium text-gray-700 border border-gray-300 bg-white hover:bg-gray-50 ${isSaving ? "opacity-60 cursor-not-allowed" : ""
              }`}
          >
            Cancel
          </button>
