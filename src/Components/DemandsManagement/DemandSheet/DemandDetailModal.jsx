...
//   }, [open, tab, profileSubTab, loadingAttached, attachedProfiles]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // 1) Optional: collect attached profileIds for this demand (best-effort filter)
        let attachedIds = new Set();
        try {
          if (row?.id) {
            const resAttached = await getAttachedProfilesByDemandId(row.id);
                    ...
                    {onbLoading ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {onboardingList.map((ob) => {
                          const statusName = ob?.onboardingStatus?.name ?? ob?.onboardingStatus ?? "";
                          const tone = onboardingTone(statusName);
                          const cardStyle = toneStyles(tone);
                          const colorText = toneTextColor(tone);

                          const demandId =
                            ob?.demand?.displayDemandId ??
                            ob?.demand?.demandId ??
                            ob?.demandId ??
                            "-";
                          const profileId = ob?.profile?.profileId ?? "-";

                          return (
                            <div
                              key={`onb-${ob?.onboardingId ?? Math.random()}`}
                              className="rounded-lg border p-3 shadow-sm"
                              style={{ ...cardStyle }}
                            >
                              <div className="flex items-center justify-between mb-2">
                                <div className="font-semibold text-[13px]" style={{ color: colorText }}>
                                  Onboarding #{ob?.onboardingId ?? "-"}
                                </div>
