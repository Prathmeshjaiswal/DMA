// src/Components/DemandsManagement/RowView.jsx
...
export default function RowView({
  ...
  // NEW: Karat pill render (Yes/No)
  const renderKarat = (value) => { // NEW
    const v = String(value || "").trim().toLowerCase();
    let cls = "bg-gray-100 text-gray-700 border border-gray-300";
    if (v === "yes") cls = "bg-green-50 text-green-700 border border-green-600/30";
    else if (v === "no") cls = "bg-gray-50 text-gray-700 border border-gray-600/30";
    return (
      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${cls}`}>
        {value || "-"}
      </span>
  ...
  return (
    <tr key={row.id ?? row.demandId ?? Math.random()} className="hover:bg-gray-50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {/* <button
                    ...
                    onClick={(e) => {
                      ...
                      if (!canUpdateDemands) {
                        message.error("You are not authorized to update demands.");
                        return;
                      }
                    ...
                    disabled={!canUpdateDemands || isLocked}
                    className={
                  ...
                  {canUpdateDemands && (
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        if (isLocked) {
                          message.warning("Finish editing the current row first.");
                          return;
                        }
      ...
      bg-white px-2 py-1 text-xs text-gray-700 hover:bg-gray-100
