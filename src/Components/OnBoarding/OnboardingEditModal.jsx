...
export default function OnboardingEditModal({ open, onClose, row, onUpdated }) {
  ...
  const initial = useMemo(() => {
    if (!row) return null;
    return {
      onboardingStatusId: row?.onboardingStatusId ?? null, // UPDATED: prefer provided id
      wbsTypeId: row?.wbsTypeId ?? null,                   // UPDATED
      bgvStatusId: row?.bgvStatusId ?? null,               // UPDATED
      offerDate: row?.offerDate || "",
      dateOfJoining: row?.dateOfJoining || "",
      ctoolId: row?.ctoolId ?? "",
      // use server field name
      pevUploadDate: row?.pevUploadDate || "",             // UPDATED
      vpTagging: row?.vpTagging || "",
      techSelectDate: row?.techSelectDate || "",
      hsbcOnboardingDate: row?.hsbcOnboardingDate || "",
    };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Fallback: if ids were not passed, try to map from names
        setForm(prev => {
          const patch = { ...prev };
      ...
      const payload = {
        ...
        ctoolId:
          form.ctoolId !== null && form.ctoolId !== undefined && String(form.ctoolId).trim() !== ""
            ? Number(form.ctoolId)
            : null,
        // correct field name the backend expects
        pevUploadDate: form.pevUploadDate || null,          // UPDATED
        vpTagging: form.vpTagging || null,
        techSelectDate: form.techSelectDate || null,
        hsbcOnboardingDate: form.hsbcOnboardingDate || null,
      };

      await editOnboarding(row.onboardingId, payload); // send path {id} + ids in body
      onUpdated && onUpdated();
      onClose && onClose();
    } finally {
      setSaving(false);
        ...
        <div className="px-5 py-4">
          {loadingDD ? (
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {/* PEV Upload */}
              <div>
                <label className={labelCls}>PEV Upload</label>
                <input
                  type="date"
                  className={inputCls}
                  value={form.pevUploadDate || ""} 
                  onChange={(e) => setField('pevUploadDate', e.target.value)}
                />
              </div>
