...
import React, { useEffect, useMemo, useState } from "react";
import Layout from "../Layout";
import { Table, Tooltip, Button, message, Alert } from "antd";
...
export default function OnboardingList() {
  ...
  function openEdit(row) {
    ...
    // 🔁 NORMALIZE what modal expects — provide ids straight from nested objects
    setEditRow({
      onboardingId: id,
      // names (optional, useful if you later show read-only labels)
      onboardingStatusName: row?.onboardingStatus?.name ?? null, // UPDATED
      wbsTypeName: row?.wbsType?.name ?? null,                   // UPDATED
      bgvStatusName: row?.bgvStatus?.name ?? null,               // UPDATED
      // ids (primary for saving)
      onboardingStatusId: row?.onboardingStatus?.id ?? null,     // UPDATED
      wbsTypeId: row?.wbsType?.id ?? null,                       // UPDATED
      bgvStatusId: row?.bgvStatus?.id ?? null,                   // UPDATED
      // dates / misc (server keys)
      offerDate: row?.offerDate ?? "",
      dateOfJoining: row?.dateOfJoining ?? "",
      ctoolId: row?.ctoolId ?? "",
      // ⚠️ backend key is pevUploadDate
      pevUploadDate: row?.pevUploadDate ?? "",                   // UPDATED (was pevUpdateDate)
  ...
  const colModel = useMemo(
    () => [
      ...
      { key: "profileId", label: "Candidate ID" },
      { key: "wbsType", label: "WBS Type" },
      { key: "offerDate", label: "Offer Date" },
      { key: "dateOfJoining", label: "DOJ" },
      { key: "ctoolId", label: "C-Tool ID" },
      { key: "bgvStatus", label: "BGV Status" },
      { key: "pevUploadDate", label: "PEV Upload" }, // UPDATED key name to match backend
      { key: "vpTagging", label: "VP Tagging" },
      { key: "techSelectDate", label: "Tech Select" },
      { key: "hsbcOnboardingDate", label: "HSBC Onboard" },
      { key: "onboardingStatus", label: "Onboarding Status" },
    ...
    const base = colModel.map((c) => {
      ...
      const renderCell = (_, row) => {
        switch (c.key) {
          ...
          case "wbsType":
            // now object {id,name}
            return row?.wbsType?.name ? (
              <span className={chip}>{row.wbsType.name}</span>
            ) : (
              "-"
            );
          ...
          case "bgvStatus":
            // now object {id,name}
            return row?.bgvStatus?.name ? (
              <span className={chip}>{row.bgvStatus.name}</span>
            ) : (
              "-"
            );
          ...
          case "onboardingStatus":
            // now object {id,name}
            return row?.onboardingStatus?.name ? (
              <span className="inline-flex items-center rounded bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">
                {row.onboardingStatus.name}
              </span>
            ) : (
              "-"
            );
          ...
          case "dateOfJoining":
            return (
              <div className="text-gray-800">{fmtDate(row?.dateOfJoining)}</div>
            );
          case "pevUploadDate": // UPDATED
            return (
              <div className="text-gray-800">{fmtDate(row?.pevUploadDate)}</div>
            );
          case "vpTagging":
            return <div className="text-gray-800">{fmtDate(row?.vpTagging)}</div>;
          case "techSelectDate":
            return (
              <div className="text-gray-800">{fmtDate(row?.techSelectDate)}</div>
            );
          case "hsbcOnboardingDate":
            return (
              <div className="text-gray-800">
      ...
      <Layout>
        <div className="p-4 md:p-6">
          ...
          <OnboardingEditModal
            open={editOpen}
            onClose={closeEdit}
            row={editRow}  // UPDATED: already normalized with ids/names
            onUpdated={() => load()}
          />
        </div>
      </Layout>
